"""
╔══════════════════════════════════════════════════════════════╗
║   AI Incidents in Financial Services — Dashboard Interativo  ║
║   Projeto Integrador · PUC-SP · Metodologia CRISP-DM         ║
╚══════════════════════════════════════════════════════════════╝

Componentes:
  • Visão Geral       — KPIs e métricas executivas
  • Explorador        — Tabelas interativas e filtros
  • Análise Estatística — H1–H4 com testes e visualizações
  ...
  • Assistente IA     — Chatbot integrado com Groq (llama-3.1-8b-instant)
...
# URL da API Flask — usa st.secrets em produção, variável de ambiente em dev,
# e localhost como último fallback.
...
def page_statistics(df: pd.DataFrame, theme: dict):
    ...
    # ── Tabs por hipótese ─────────────────────────────────────────────────────
    tabs = st.tabs(["H1 — Concentração", "H2 — Viés", "H3 — Severidade", "H4 — Tendência",
                    "📊 Descritiva Completa"])
...
def page_api(theme: dict):
    ...
    if status["ok"]:
        ...
        st.warning(
            f"⚠️ API offline (`{API_BASE}`). Em produção: verifique a URL em st.secrets['API_BASE_URL']."
        )
...
def _get_groq_key() -> str:
    """Resolve a chave Groq: st.secrets → env var → session_state → string vazia."""
    # 1. Streamlit Cloud (produção)
    try:
        return st.secrets["GROQ_API_KEY"]
    except Exception:
        pass
    # 2. Variável de ambiente local (.env)
    env_val = os.environ.get("GROQ_API_KEY", "")
    if env_val:
        return env_val
    # 3. Chave inserida manualmente via UI
    return st.session_state.get("groq_key_manual", "")


def _call_groq(messages: list, system_prompt: str, groq_key: str) -> str:
    """
    Chama a API Groq com o modelo llama-3.1-8b-instant.
    Usa a interface REST diretamente via requests — sem dependência do SDK da Groq
    (evita conflito com o pacote openai que pode estar instalado).

    Fallback 1: se Groq falhar com rate-limit (429), tenta novamente após 2s.
    Fallback 2: se falhar com erro de autenticação (401), retorna mensagem clara.
    Fallback 3: qualquer outro erro → retorna mensagem de erro + detalhes.
    """
    ...
    for attempt in range(2):  # até 2 tentativas
        try:
            ...
            if resp.status_code == 200:
                ...
                if attempt == 0:
                    _time.sleep(2)
                    continue
                return "⚠️ Limite de requisições da Groq atingido. Aguarde alguns segundos e tente novamente."

            elif resp.status_code == 401:
                return ("❌ Chave Groq inválida ou expirada.\n\n"
                        "Verifique em: https://console.groq.com/keys")

            else:
                return f"❌ Erro Groq [{resp.status_code}]: {resp.text[:200]}"

        except requests.exceptions.Timeout:
            return "⚠️ Timeout na API Groq (>30s). Verifique sua conexão e tente novamente."
        except requests.exceptions.ConnectionError:
            return "❌ Sem conexão com a API Groq. Verifique sua internet."
        except Exception as e:
            return f"❌ Erro inesperado: {str(e)}"

    return "❌ Falha após 2 tentativas. Usando resposta local."


def page_chatbot(df: pd.DataFrame, theme: dict):
    section_header(
        "🤖 Assistente IA",
        "Chat inteligente sobre os dados — Groq · llama-3.1-8b-instant · Gratuito"
    )

    # ── Resolução automática de chave ────────────────────────────────────────
    # Ordem de prioridade: st.secrets → variável de ambiente → input manual
    auto_key = _get_groq_key()
    groq_configured = bool(auto_key)

    # ── Painel de configuração (só aparece se chave não foi resolvida auto) ──
    if not groq_configured:
        with st.expander("⚙️ Configurar API Groq (gratuita)", expanded=True):
            st.markdown(
                "**A Groq é 100% gratuita para uso acadêmico.**\n\n"
            ...
            manual_key = st.text_input(
                ...
                type="password",
                placeholder="gsk_...",
                help="A chave não é armazenada permanentemente — apenas na sessão atual.",
            )
            if manual_key:
                st.session_state["groq_key_manual"] = manual_key
                st.success("✅ Chave configurada para esta sessão")
                groq_configured = True
                auto_key = manual_key
            else:
                st.info(
                    "Sem chave → o assistente responde em modo offline "
                    "(respostas baseadas nos dados do projeto)."
                )
    else:
        # Chave resolvida automaticamente — mostra badge discreto
        source = "st.secrets" if "GROQ_API_KEY" in (st.secrets if hasattr(st, "secrets") else {}) else "variável de ambiente"
        st.markdown(
            f'<div style="font-size:0.8rem;color:#22c55e;margin-bottom:12px;">'
            f'✅ Groq configurada via {source} · modelo: llama-3.1-8b-instant</div>',
            unsafe_allow_html=True,
        )
    ...
    def build_context(df: pd.DataFrame) -> str:
        ...
        return f"""Você é um especialista em análise de risco de IA em serviços financeiros, assistindo estudantes da PUC-SP.

DADOS DO PROJETO (AI Incident Database — recorte financeiro):
- Total de incidentes analisados: {total}
- Período coberto: {years}
- Distribuição de severidade: {sev_dist}
- Top 3 aplicações com mais incidentes: {app_dist}
- Taxa de investigação regulatória: {inv_rate:.1f}%

METODOLOGIA (CRISP-DM — 4 notebooks):
- NB1: Coleta via API AIID, limpeza, feature engineering (8 variáveis derivadas por keywords)
- NB2: Análise estatística — H1 (qui-quadrado aderência), H2 (qui-quadrado independência),
       H3 (qui-quadrado + regressão logística + Odds Ratio), H4 (Spearman + OLS)
- NB3: ML — Logistic Regression (baseline), Random Forest, XGBoost + SHAP values
- NB4: API RESTful Flask com 9 endpoints (dados, estatísticas e predições ML)

REGRAS DE RESPOSTA:
- Responda sempre em português brasileiro
- Use markdown com bullet points para listas
- Seja objetivo, preciso e cite dados reais quando disponíveis
- Para perguntas fora do escopo do projeto, redirecione gentilmente
""".strip()
    ...
    cols = st.columns(3)
    for i, (col, sugg) in enumerate(zip(cols * 2, suggestions)):
        with col:
            if st.button(
                sugg[:55] + "…" if len(sugg) > 55 else sugg,
                key=f"sugg_{i}",
                use_container_width=True,
            ):
    ...
    if user_input or st.session_state.get("send_pending"):
        ...
        # Resolver chave (pode ter sido configurada agora via UI)
        resolved_key = _get_groq_key()

        if resolved_key:
            # ── Groq (principal) ─────────────────────────────────────────────
            with st.spinner("⚡ Gerando resposta com Groq..."):
                answer = _call_groq(
                    messages      = [{"role": m["role"], "content": m["content"]}
                                     for m in st.session_state["chat_messages"]],
                    system_prompt = build_context(df),
                    groq_key      = resolved_key,
                )

            # Fallback para modo local se Groq retornou erro
            if answer.startswith("❌") or answer.startswith("⚠️"):
                fallback = _local_chatbot(question, df)
                answer   = f"{answer}\n\n---\n**Resposta offline:**\n\n{fallback}"
    ...
    if st.session_state.get("chat_messages"):
        ...
        with col_clear:
            if st.button("🗑️ Limpar conversa"):
                ...
                st.rerun()
        with col_info:
            model_label = "llama-3.1-8b-instant via Groq" if resolved_key else "modo offline"
            st.markdown(
                f'<div style="font-size:0.75rem;color:#94a3b8;padding-top:8px;">'
                f'Usando: {model_label}</div>',
                unsafe_allow_html=True,
            )
...
def _local_chatbot(question: str, df: pd.DataFrame) -> str:
    ...
    return (
        ...
        "💡 *Configure uma chave Groq gratuita em console.groq.com para respostas mais ricas.*"
    )
